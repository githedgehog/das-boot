...
# install our CRDs
helm --kubeconfig /etc/rancher/k3s/k3s.yaml upgrade --install hh-fabric-api --force --version=v0.16.0 oci://registry.local:5000/githedgehog/helm-charts/fabric-api
if [ $? -ne 0 ]; then
    exit 4
fi
